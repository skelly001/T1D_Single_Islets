# ==============================================================================
# Gene Ontology Cluster Scoring Function Definition
# ==============================================================================
# Script: 8a_1-clustGO_Function_GOScore.R
# Description: Defines the clustGO() function for Gene Ontology enrichment
#              analysis and cluster quality scoring. Uses gProfiler2 (gost) to
#              perform GO enrichment testing with g_SCS multiple testing correction.
#              Calculates F-beta scores to evaluate clustering quality based on
#              annotated genes and GO term enrichment.
#
# Input: None (function definition file)
# Output: None (provides clustGO function for downstream analysis)
# ==============================================================================

# clustGO Function Definition ---------------------------------------------
...
clustGO <- function(
	...
	if (S_type == "Summary") {
		...
		Rep_GO_terms <- as.data.frame(gostres$result) %>%
			...
			group_by(query) %>%
			slice_max(order_by = FBscore, n = 1, with_ties = T) %>%
			dplyr::select(
				-effective_domain_size,
				-source_order,
				-parents,
			...
			mutate(
				term_name = str_c(term_name, collapse = ","),
		...
		complete_results <- full_join(
			...
			distinct(intersection, .keep_all = T) %>%
			rename(Gene = intersection, GO_term_pvalue = p_value) %>%
		...
		result <- list(temp, All_GO_terms, Rep_GO_terms, complete_results)
		names(result) <- listN
		return(result)
	}
	return(temp)
